...
imbalance   = (100 - soh_true) / 100 * 0.3 + rng.uniform(0, 0.05, n)
base_v      = 3.80 + soh_true / 100 * 0.04  # Healthy=3.84V, degraded=3.80V

c1_v = base_v + rng.uniform(-0.02, 0.02, n)
c2_v = base_v + rng.uniform(-0.02, 0.02, n)
c3_v = base_v - imbalance + rng.uniform(-0.01, 0.01, n)  # Weakest cell
c4_v = base_v + rng.uniform(-0.02, 0.02, n)

# Temperature rises as internal resistance increases with aging
base_temp   = 33.0 + (100 - soh_true) / 100 * 12.0
max_temp_c  = base_temp + rng.uniform(0, 4, n)

current_a   = rng.uniform(-3.0, -0.5, n)
vibration   = rng.uniform(0.01, 0.06, n)
gas_ppm     = 4.0 + (100 - soh_true) / 100 * 8.0 + rng.uniform(0, 2, n)
