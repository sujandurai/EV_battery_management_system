...
def hard_rules(data):
    ...
    for i,val in enumerate(v):
        if val < CELL_VOLT_MIN: r.append(f"CELL_{i+1}_VOLT_LOW({val:.2f}V)")
    for i,val in enumerate(t):
        if val > TEMP_CRITICAL:  r.append(f"CELL_{i+1}_TEMP_HIGH({val:.1f}C)")
    if p.get("gas_ppm",0) > GAS_CRITICAL: r.append(f"GAS_HIGH({p['gas_ppm']}ppm)")
...
def on_message(client, userdata, message):
    ...
    try:
        ...
        # Calculate cell trust scores (healthy baseline ~99.0%)
        cell_trust_pct = []
        for idx in range(4):
            v_val = volts[idx] if idx < len(volts) else 0.0
            t_val = temps[idx] if idx < len(temps) else 0.0
            trust = 99.0
            if v_val < 3.0:
                trust -= ((3.0 - v_val) / 1.5) * 80.0
            if t_val > 45.0:
                trust -= ((t_val - 45.0) / 25.0) * 80.0
            trust = max(5.0, min(99.0, trust))
            cell_trust_pct.append(round(trust, 1))

        # Check if any cell trust indicates a fault (< 30%)
        any_cell_fault = any(t_score < 30.0 for t_score in cell_trust_pct)
        is_anomaly     = (anom_label == -1) or rule_fault or any_cell_fault
        alert_reason   = rule_reason if rule_fault else ("CELL_TRUST_FAULT" if any_cell_fault else ("MODEL_ANOMALY" if anom_label == -1 else ""))
        if is_anomaly: _anomaly_count += 1
        ...
        trust_msg = json.dumps({"status": "FAULT" if is_anomaly else "OK"})
...
def main():
    ...
    global _anom_sess, _anom_inp, _soh_sess, _soh_inp, _pinn_sess, _pinn_inp
